...
d = struct('TIME',meta.nrecs, ... % TIME is always first dimension
           ...
           'LATITUDE', 1, 'LONGITUDE', 1); % LATITUDE and LONGITUDE are singleton dimensions

%% Define Variables included for all data types
varStruct = struct('xType', [], 'dimids', [], 'Atts', []); % Empty variable definition structure
% We always have TIME, DEPTH, LATITUDE and LONGITUDE
v = struct('TIME', varStruct,'DEPTH', varStruct,'LATITUDE', varStruct,'LONGITUDE', varStruct);
...
v.TIME.Atts = struct(...
      ...
      'axis', 'T', ...
      'long_name', 'time of each measurement',...
      'valid_min', 0.0,...
      'valid_max', 90000.0,...
...
v.LATITUDE.xType = 'NC_FLOAT';
v.LATITUDE.dimids = {'LATITUDE'};
v.LATITUDE.Atts = struct(...
...
v.LONGITUDE.xType = 'NC_FLOAT';
v.LONGITUDE.dimids = {'LONGITUDE'};
v.LONGITUDE.Atts = struct(...
