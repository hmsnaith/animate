...
for j=1:nvar;
	...
	nc{['QC_PRES' int2str(v(j))]}.description=['Quality Marker for nominal depth of ' int2str(v(j)) ' metres'];
	nc{['QC_PRES' int2str(v(j))]}.missing_value= 9;
	nc{['QC_PRES' int2str(v(j))]}.valid_min= 0;
	nc{['QC_PRES' int2str(v(j))]}.valid_max= 9;
	nc{['QC_PRES' int2str(v(j))]}.long_name='Quality Marker for Pressure';
	end;
end;
...
nc{'Depthx'}(1)=0;
nc{'TEMP'}(:,:,1,1)=T';
nc{'Conductivity'}(:,:,1,1)=C';
nc{'PRES'}(:,:,1,1)=P';
nc{'PSAL'}(:,:,1,1)=S';
nc{'QC_TEMP'}(:,:,1,1)=TQ';
nc{'QC_Conductivity'}(:,:,1,1)=CQ';
nc{'QC_PRES'}(:,:,1,1)=PQ';
nc{'QC_PSAL'}(:,:,1,1)=SQ';
